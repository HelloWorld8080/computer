...
class MvCamera():
    ...
    def MV_CC_GetExposureAutoMode(self):
        stEnumValue = MVCC_ENUMVALUE()
        memset(byref(stEnumValue), 0, sizeof(MVCC_ENUMVALUE))
        ret = MvCamCtrldll.MV_CC_GetExposureAutoMode(self.handle,stEnumValue)
        if ret != 0:
    ...
    def MV_CC_SetExposureAutoMode(self, nValue):
        ...
        # C原型:int MV_CC_SetEnumValueByString(void* handle,char* strKey,char* sValue)
        return MvCamCtrldll.MV_CC_SetExposureAutoMode(self.handle, c_uint32(nValue))
    ...
    def MV_CC_GetGainMode(self):
        stEnumValue = MVCC_ENUMVALUE()
        memset(byref(stEnumValue), 0, sizeof(MVCC_ENUMVALUE))
        ret = MvCamCtrldll.MV_CC_GetGainMode(self.handle,stEnumValue)
        if ret != 0:
            raise Exception("获取ExposureAutoMode失败 ! 报错码 ret[0x%x]" % (ret))
        return stEnumValue.nCurValue
    
    def MV_CC_GetSharpness(self):
        stParam = MVCC_INTVALUE_EX()
        memset(byref(stParam), 0, sizeof(MVCC_INTVALUE_EX))
        ret = MvCamCtrldll.MV_CC_GetSharpness(self.handle,stParam)
        if ret != 0:
